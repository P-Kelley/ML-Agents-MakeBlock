...
public class WallJumpAgent : Agent
{
    ...
    public void MoveAgent(ActionSegment<int> act)
    {
        ...
        var makeBlockAction = act[4];

        if(makeBlockAction == 1){
            AddReward(-0.001f);
            var madeBlock = Instantiate(shortBlock, m_AgentRb.position, Quaternion.identity);
        }
    ...
    //Removes all extra blocks after stage
    void ResetBlock()
    {

    GameObject[] allObjects = GameObject.FindGameObjectsWithTag("block");
    foreach(GameObject obj in allObjects) {
        if(obj.transform.name == "shortBlock(Clone)"){
            Destroy(obj);
        }
}
    }
